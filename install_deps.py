...
if __name__ == "__main__":
    ...
    if not os.path.exists(APPENGINE_TARGET_DIR):
        ...
        print(
            'Not updating SDK as it exists. Remove {} and re-run to get the latest SDK'.format(APPENGINE_TARGET_DIR))

    # Remove all folders and files, we leave symlinks as they will be pointing
    # to submodules
    filenames = (filename for filename in os.listdir(
        TARGET_DIR) if filename not in FILES_IGNORED_FOR_DELETION)
